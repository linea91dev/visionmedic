...
<body>
    <?php 
        $info_date = $this->db->get_where('cart', array('cart_id' => $sale_id))->row()->date;
        $info_productos = $this->db->get_where('cart', array('cart_id' => $sale_id))->row()->products;
        $info_total = $this->db->get_where('cart', array('cart_id' => $sale_id))->row()->total;
        ...
        <table cellpadding="0" cellspacing="0"  style="width: 100%; text-align: left; margin-top:50px;">
            ...
            <tr>
                <td colspan="4" style="padding:10px;font-size: 17px;text-align:right;border:1px; border-top:1px solid black">
                    <b>TOTAL: </b>
                    <?php echo $this->db->get_where('clinic', array('clinic_id' => $this->session->userdata('current_clinic')))->row()->currency;?>. <?php echo number_format($info_total,'2','.',',');?>
                </td>
            </tr>
            </tbody>
        </table>
