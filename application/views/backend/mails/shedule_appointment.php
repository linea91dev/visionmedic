...
<body style="background-color: #222533; padding: 20px; font-family: font-size: 14px; line-height: 1.43; font-family: &quot;Helvetica Neue&quot;, &quot;Segoe UI&quot;, Helvetica, Arial, sans-serif;">
        <div style="max-width: 600px; margin: 0px auto; background-color: #fff; box-shadow: 0px 20px 50px rgba(0,0,0,0.05);">
           ...
           <h2 class="title" style="font-size: 42px; font-weight: 600; font-style: normal; font-stretch: normal; line-height: normal; letter-spacing: normal; text-align: center; color: #000000; margin: 25px 0;">Cita programada</h2>
            <div style="padding: 60px 70px; border-top: 1px solid rgba(0,0,0,0.05);">
                ...
                <div style="color: #636363; font-size: 14px;">
                    <p>Tu cita a sido programada para el <b><?php echo $appointment['day'];?> de <?php setlocale(LC_TIME, 'es_ES');  $numero = $appointment['month'];  $fecha = DateTime::createFromFormat('!m', $numero); $mes = strftime("%B", $fecha->getTimestamp()); echo $mes?> a las <?php echo $appointment['time'];?> horas</b>,
                    para mas información puedes comunicarte al nuestro numero <b><?php echo $this->db->get_where('clinic', array('clinic_id' => $appointment['clinic_id']))->row()->phone;?></b> o al correo electronico <b><?php echo $this->db->get_where('clinic', array('clinic_id' => $appointment['clinic_id']))->row()->email;?></b></p>
                </div>
                <h4 style="margin-bottom: 100px; margin-top: 100px;">!Te esperamos!</h4>
                <div style="color: #A5A5A5; font-size: 12px;">
                    <p>Si tienes alguna pregunta o duda con respecto a este correo electrónico. Por favor escríbenos a <a href="mailto:<?php echo $this->db->get_where('clinic', array('clinic_id' => $appointment['clinic_id']))->row()->email;?>" style="text-decoration: underline; color: #4B72FA;"><?php echo $this->db->get_where('clinic', array('clinic_id' => $appointment['clinic_id']))->row()->email;?></a></p>
                </div>
            </div>
