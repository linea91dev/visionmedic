...
<body style="font-family: Helvetica, Arial, sans-serif; color: #000;">
	<div class="container" style="width: 100%; max-width: 536px; margin: 0 auto;">
		...
		<div class="order" style="-webkit-font-smoothing: antialiased; font-size: 16px;">
			<h2 class="title" style="font-size: 42px; font-weight: 600; font-style: normal; font-stretch: normal; line-height: normal; letter-spacing: normal; text-align: center; color: #000000; margin: 25px 0;">Receta electrónica</h2>
			<div class="message" style="font-size: 16px; line-height: 1.5; text-align: center; color: #333333; margin-bottom: 20px;">Hola, <b><?php echo $this->db->get_where('patient', array('patient_id' => $patient_id))->row()->first_name;?></b>. Recibes este correo porque tu médico ha enviado la receta de tu cita. Puedes encontrar mas detalles en el documento al final de este correo.
			</div>
				...
				<tr>
					<td style="padding: 10px 0;">Si tienes alguna duda con respecto a la receta, puedes contactarnos al nuestro numero <b><?php echo $this->db->get_where('clinic', array('clinic_id' => $this->session->userdata('current_clinic')))->row()->phone;?></b> o por correo electrónico <b><?php echo $this->db->get_where('clinic', array('clinic_id' => $this->session->userdata('current_clinic')))->row()->email;?></b></td>
				</tr>
			</table>	
			<hr class="separator--line" style="border: solid 0.5px rgba(0, 0,0, 0.1);">
		</div>
